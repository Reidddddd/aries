...
public class AlterMemstoreFlushSize extends AlterBase {

  private final static long AT_LEAST_SIZE = Constants.ONE_MB * 10;

  private long flush_size;
  private HTableDescriptor descriptor;

  public AlterMemstoreFlushSize() {}

  protected void alter(TableName table, HColumnDescriptor family) throws Exception {
    flush_size = RANDOM.nextLong();
    if (flush_size < AT_LEAST_SIZE) {
      flush_size = AT_LEAST_SIZE;
    }
    descriptor.setMemStoreFlushSize(flush_size);
  ...
  protected void preAlter(TableName table, HColumnDescriptor family) throws Exception {
    ...
    flush_size = descriptor.getMemStoreFlushSize();
    LOG.info(preLogMessage("max file size", table, family, flush_size));
  }

  protected void postAlter(TableName table, HColumnDescriptor family) throws Exception {
    super.postAlter(table, family);
    LOG.info(postLogMessage("block size", table, family, flush_size));
  }
